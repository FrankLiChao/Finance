...
+ (instancetype)footer
{
    MJRefreshFooterView * f = [[MJRefreshFooterView alloc] init];
    f.isHeader = NO;
...
{
    // 内容的高度
    CGFloat contentHeight = self.scrollView.contentSizeHeight+10;
    // 表格的高度
    CGFloat scrollHeight = self.scrollView.height - self.scrollViewOriginalInset.top - self.scrollViewOriginalInset.bottom;
    // 设置位置和尺寸
    if (self.tag == shopDetailFooterTag) {
        self.y = MAX(contentHeight, scrollHeight) - MJRefreshViewHeight;
    }
    else{
...
- (void)adjustStateWithContentOffset
{
    ...
    if (self.tag == shopDetailFooterTag) {
        happenOffsetY = [self happenOffsetY]-(MJRefreshViewHeight/2);
    }
...
//    CGRect rec = self.acImgView.frame;
//    rec.origin.y = self.statusLabel.frame.origin.y;
//    self.acImgView.frame = rec;
//    
//    CGRect rec1 = self.activityView.frame;
//    rec1.origin.y = self.statusLabel.frame.origin.y;
//    self.activityView.frame = rec1;
