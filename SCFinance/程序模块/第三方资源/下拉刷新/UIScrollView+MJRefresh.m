...
- (void)whiteV
{
    UIView * whiteV = [[UIView alloc]initWithFrame:CGRectMake(0, -DeviceMaxHeight, DeviceMaxWidth, DeviceMaxHeight)];
...
/**
 *  商品详情添加一个上拉刷新尾部控件（特殊的）
 *
 *  @param target 目标
 *  @param action 回调方法
 */
- (void)addFooterWithTag:(NSInteger)tag target:(id)target action:(SEL)action
{
    // 1.创建新的footer
    if (!self.footer) {
        MJRefreshFooterView *footer = [MJRefreshFooterView footer];
        footer.tag = shopDetailFooterTag;
        [self addSubview:footer];
